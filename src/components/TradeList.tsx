...
export const TradeList: React.FC<TradeListProps> = React.memo(
  ({ trades, onDelete, onEdit }) => {
    ...
    const filteredAndSortedTrades = useMemo(() => {
      ...
      if (timeRange !== "All") {
        ...
        result = result.filter((t) => {
          ...
          if (timeRange === "Week")
            return isThisWeek(tradeDate, { weekStartsOn: 1 });
    ...
    const SortIcon = ({ column }: { column: SortKey }) => {
      if (sortConfig.key !== column)
        return <ArrowUpDown size={12} className="opacity-20" />;
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5">
                <th
                  className="p-4 data-grid-header cursor-pointer hover:bg-white/5 transition-colors"
                  onClick={() => handleSort("trade_date")}
                >
                  <div className="flex items-center gap-1">
                    Date <SortIcon column="trade_date" />
                  </div>
                </th>
                <th
                  className="p-4 data-grid-header cursor-pointer hover:bg-white/5 transition-colors"
                ...
                >
                  <div className="flex items-center gap-1">
                    Type <SortIcon column="asset_type" />
                  </div>
                </th>
                <th
                  className="p-4 data-grid-header cursor-pointer hover:bg-white/5 transition-colors"
                ...
                <th
                  className="p-4 data-grid-header text-right cursor-pointer hover:bg-white/5 transition-colors"
